# backends/sts3215/protocol.py
#
# Low-level FT-SCS servo protocol implementation.
...
def ping(ser: serial.Serial, servo_id: int) -> bool:
    ...
    try:
        ...
        print(f"[STS3215 PING] Error during ping for servo {servo_id}: {e}")
        return False
...
def write_register_byte(ser: serial.Serial, servo_id: int, register_address: int, value: int) -> bool:
    ...
    try:
        ...
        print(f"[STS3215] Error writing byte to servo {servo_id} register {hex(register_address)}: {e}")
        return False
...
def write_register_word(ser: serial.Serial, servo_id: int, register_address: int, value: int) -> bool:
    ...
    try:
        ...
        print(f"[STS3215] Error writing word to servo {servo_id} register {hex(register_address)}: {e}")
        return False
...
def read_position(
    ...
    try:
        ...
        print(f"[STS3215 ReadPos] Servo {servo_id}: Error: {e}")
        return None
...
def sync_write_goal_pos_speed_accel(ser: serial.Serial, servo_data_list: list[tuple[int, int, int, int]]) -> None:
    ...
    try:
        ...
        print(f"[STS3215 SyncWrite] Error: {e}")
...
def sync_read_positions(
    ...
    try:
        ...
        if len(response_data) < bytes_to_read:
            print(f"[STS3215 SyncRead] WARNING: Expected {bytes_to_read} bytes, got {len(response_data)}.")
        ...
        if expected_ids:
            ...
            print(f"[STS3215 SyncRead] No response from IDs: {missing}")
            if alert_callback:
                for sid in missing:
        ...
        print(f"[STS3215 SyncRead] Error: {e}")
        return {}
    finally:
...
def sync_read_block(
    ...
    try:
        ...
        print(f"[STS3215 SyncReadBlk] Error: {e}")
        return {}
    finally:
        if timeout_s is not None and original_timeout is not None:
            ser.timeout = original_timeout
...
def calibrate_middle_position(ser: serial.Serial, servo_id: int) -> bool:
    ...
    try:
        ...
        print(f"[STS3215] Error sending calibrate command to servo {servo_id}: {e}")
        return False
...
def factory_reset(ser: serial.Serial, servo_id: int) -> bool:
    ...
    try:
        ...
        print(f"[STS3215] Error sending factory reset to servo {servo_id}: {e}")
        return False
...
def restart(ser: serial.Serial, servo_id: int) -> bool:
    ...
    try:
        ...
        print(f"[STS3215] Error sending restart to servo {servo_id}: {e}")
        return False
...
def write_angle_limits(
    ...
    if not write_register_byte(ser, servo_id, config.SERVO_ADDR_WRITE_LOCK, 0):
        print(f"[STS3215] Failed to unlock EEPROM for servo {servo_id}")
        return False
    time.sleep(0.01)
